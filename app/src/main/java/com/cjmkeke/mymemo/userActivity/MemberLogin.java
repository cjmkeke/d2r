...
public class MemberLogin extends AppCompatActivity {
    ...
    private TextView login, findPassword;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findPassword = findViewById(R.id.tv_re_password);
        findPassword.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

                if (TextUtils.isEmpty(email.getText().toString())){
                    Toast.makeText(MemberLogin.this, "이메일을 입력해주세요.", Toast.LENGTH_SHORT).show();
                } else {
                    firebaseAuth.sendPasswordResetEmail(email.getText().toString()).addOnSuccessListener(new OnSuccessListener<Void>() {
                        @Override
                        public void onSuccess(Void unused) {
                            Toast.makeText(MemberLogin.this, "메일로 비밀번호 전송되었습니다.", Toast.LENGTH_SHORT).show();
                        }
                    }).addOnFailureListener(new OnFailureListener() {
                        @Override
                        public void onFailure(@NonNull Exception e) {
                            if (e instanceof FirebaseAuthInvalidCredentialsException){
                                Toast.makeText(MemberLogin.this, "이메일 형식을 지켜주세요.", Toast.LENGTH_SHORT).show();
                            } else if (e instanceof FirebaseAuthInvalidUserException){
                                Toast.makeText(MemberLogin.this, "인증된 이메일이 없습니다.", Toast.LENGTH_SHORT).show();
